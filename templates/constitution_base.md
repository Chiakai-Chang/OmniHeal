...
## 專案自身原則（Project Constitution）
> 來源：Phase 0 Step 2.5 從 file_index.md 語義掃描後全量讀取所有理念候選檔提取。
> 僅收錄 `✓ VERIFIED`（有直接原文引用）的原則。
> Phase 1 掃描時，違反這些原則的 finding 標注 `[理念對齊違反：來源 §章節]`，severity 自動升一級。

| 原則 | 來源 | 引用原文（片段）| 對齊等級 |
|------|------|--------------|---------|
| （Phase 0 Step 2.5 填入，若無則填「未偵測到專案自身原則」）| — | — | — |

**CLAUDE.md / AGENTS.md**：「掃描規則類」條目視為上層指令（強制）；「Agent 行為類」條目跳過。

**理念落差診斷門檻**：同類型「理念對齊違反」findings > 30% 時，Phase 1.5 改輸出「理念落差診斷」。
